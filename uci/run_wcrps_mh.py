...
class CRPSRegressor(nn.Module):
    ...
    def forward(self, x, n_samples=10):
        ...
        return samples.reshape(*samples.shape[:-2], -1), torch.exp(weights_log_softmax+np.log(weights_log_softmax.shape[-1]))
...
for dataset_name in dataset_names:
    ...
    for split_ix, (index_train, index_test) in enumerate(tqdm(splits)):
        ...
        model = CRPSRegressor(X_train.shape[1], heads).to(device)
        ...
        for k in range(max_epoch):
            ...
            with torch.device(X_train_s.device):
                ...
                val_nll = crps.nll_gpu_weighted(preds_gpu, weights_gpu, y_gpu, max_pilot_samples=1000).mean().item()
        ...
        val_pred_samples, val_pred_weights = crps.predict(model, scalers, X_validation, n_samples=valid_samples, batch_size=128)
        with torch.device(X_train_s.device):
            preds_gpu, weights_gpu, y_gpu = torch.tensor(val_pred_samples).float(), torch.tensor(val_pred_weights).float(), torch.tensor(y_validation.reshape(-1,1)).float()
            val_crps = crps.crps_loss_weighted(preds_gpu, weights_gpu, y_gpu).mean().item()
        ...
        test_pred_samples, test_pred_weights = crps.predict(model, scalers, X_test, n_samples=valid_samples, batch_size=128)
        with torch.device(X_train_s.device):
