...
for dataset_name in dataset_names:
    ...
    for split_ix, (index_train, index_test) in enumerate(tqdm(splits)):
        ...
        model = CRPSRegressor(X_train.shape[1], heads, layers).to(device)
        ...
        for k in range(max_epoch):
            ...
            model.eval()
            val_pred_samples, val_pred_weights = crps.predict(model, scalers, X_validation, n_samples=valid_samples//(heads*layers), batch_size=128)
        ...
        test_pred_samples, test_pred_weights = crps.predict(model, scalers, X_test, n_samples=valid_samples//(heads*layers), batch_size=128)
        with torch.device(X_train_s.device):
