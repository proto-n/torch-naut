...
if __name__ == "__main__":
    ...
    for k in range(100):
        for ixs in tqdm(get_batch_ixs(X_train_s, batch_size, permute=True)):
            ...
            loss = crps.crps_loss_mv(x, y).sum()
            ...
            scheduler.step()
        with torch.no_grad():
            samples = net(torch.tensor([[0.25]]).float().to(device), n_samples=10000)
        plt.figure()
        plt.hist2d(*samples.squeeze().T.cpu().numpy(), bins=100, cmap='inferno')
        plt.gca().set_aspect(1)
        plt.savefig("results/crps/%d.png"%k)
        plt.show()
        plt.close()
        ...
        if k%10==9:
            ...
            for i, ax in zip(np.linspace(0, 2, 4*4+1)[1:]**2, axs.reshape(-1)):
                ...
                with torch.no_grad():
                    samples = net(torch.tensor(irt).float().to(device), n_samples=10000)
                ...
                ax.set_facecolor('k')
            fig.tight_layout()
            plt.savefig("results/crps/%d_big.png"%k)
            plt.show()
            plt.close()
            torch.save(net.state_dict(), "results/crps/%d.pt"%k)
